        ...
        Button(onClick = {
            selectedString?.let {
                result = when (selectedMethod) {
                    1 -> MD5Utils.changeMD5Init(it)
                    2 -> MD5Utils.changeMD5Update(it)
                    3 -> MD5Utils.changeConstant(it)
                    4 -> MD5Utils.javaMD5(it)
                    else -> MD5Utils.md5(it)
                }
                Log.d("MD5", "$selectedString -> $result")
...
fun DropdownMenu(selectedIndex: Int, onSelectedChange: (Int) -> Unit) {
    val options = listOf("标准MD5", "修改MD5Init常量", "修改MD5Update", "修改宏常量", "标准 Java MD5")

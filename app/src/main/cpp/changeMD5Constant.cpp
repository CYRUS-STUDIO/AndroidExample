...
/* forward declaration */
static void Transform_(UINT4 *buf, UINT4 *in);
 ...
 */
void MD5_Init_(MD5_CTX *mdContext) {
 ...
 */
void MD5_Update_(MD5_CTX *mdContext, unsigned char *inBuf, unsigned int inLen) {
    UINT4 in[16];
    int mdi;
    unsigned int i, ii;
 ...
 */
void MD5_Final_(unsigned char hash[], MD5_CTX *mdContext) {
    UINT4 in[16];
    int mdi;
    unsigned int i, ii;
    unsigned int padLen;
    ...
    MD5_Update_(mdContext, PADDING, padLen);
...
// 使用 extern 声明外部函数
extern std::string bytesToHex(const uint8_t *bytes, size_t length);
